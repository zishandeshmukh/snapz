...
// Use connection pooling for Supabase free tier
const supabase = createClient(
  ...
  process.env.SUPABASE_SERVICE_KEY,
  {
    db: {
      schema: 'public',
      pool: { min: 0, max: 2 } // Stay within free tier limits
    }
  }
);
...
// Aggressive rate limiting for 100 users
...
app.use((req, res, next) => {
  ...
  const maxHits = req.path.includes('receive_data') ? 2 : 10;
...
// Job #1: AI Summarization
app.post('/receive_data', async (req, res) => {
  const { text, user_id, source } = req.body;
  
  // Validate
  if (!text || text.length > 5000) {
    return res.status(400).json({ error: 'Text too long' });
  }
  if (!user_id || user_id.length > 50) {
    return res.status(400).json({ error: 'Invalid user' });
  }

  try {
    // Call Gemini with system prompt for structured data
    const response = await axios.post(
      `${GEMINI_URL}?key=${GEMINI_KEY}`,
      {
        contents: [{
          parts: [{
            text: `ANALYZE_AND_RETURN_JSON_ONLY:
...
app.post('/searchNLPSql', async (req, res) => {
  ...
  try {
      ...
      `${GEMINI_URL}?key=${GEMINI_KEY}`,
      {
        contents: [{
          parts: [{
            ...
            Use Supabase PostgREST syntax. Always include user_id security.`
          }]
        }]
      }
    );
    ...
    const aiText = response.data.candidates[0].content.parts[0].text;
    const cleaned = aiText.replace(/```json\n|\n```/g, '').trim();
    const { filter } = JSON.parse(cleaned);
...
app.listen(PORT, () => console.log(`🚀 Server ready for 100 users on port ${PORT}`));import express from 'express';
import cors from 'cors';
import dotenv from 'dotenv';
import { createClient } from '@supabase/supabase-js';
import axios from 'axios';
...
const PORT = process.env.PORT || 10000;
app.listen(PORT, () => console.log(`🚀 Server ready for 100 users on port ${PORT}`));
