...
const AuthScreen = () => {
  ...
  const handleAuth = async () => {
    ...
    try {
      if (isSignUp) {
        // ✅ Try to sign in FIRST (they might already exist)
        ...
        if (signInError) {
          // If sign in fails, they might be new - try sign up
    ...
    >
      <View style={styles.innerContainer}>
        ...
        <TextInput
          ...
          onChangeText={(text) => {
            setEmail(text);
            setShowResend(false); // Hide resend if user types again
          }}
        ...
        >
          <Text style={styles.buttonText}>
            {isSignUp ? 'Sign Up / Login' : 'Login'} {/* ✅ Shows dual action */}
          </Text>
        </TouchableOpacity>

        {/* ✅ NEW: Resend confirmation button */}
        {showResend && (
          <TouchableOpacity 
        ...
        <TouchableOpacity onPress={() => {
          ...
          setShowResend(false); // Reset when toggling
        }} disabled={loading}>
