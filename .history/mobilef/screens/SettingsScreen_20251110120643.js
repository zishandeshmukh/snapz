...
export default function SettingsScreen() {
  ...
  const openWebApp = () => {
    if (FRONT_URL) {
      Linking.canOpenURL(FRONT_URL).then(supported => {
        if (supported) {
          Linking.openURL(FRONT_URL);
        } else {
          Alert.alert('Error', `Cannot open URL: ${FRONT_URL}`);
        }
      });
    } else {
      Alert.alert('Error', 'FRONT_URL not configured');
    }
  };
